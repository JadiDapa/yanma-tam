...
import { z } from "zod";
import { updateAttendanceListFull } from "@/app/actions/attendance-list.action";
...
export default function AttendanceDetailForm({
  ...
  const [rows, setRows] = useState<EntryRow[]>(() =>
    // Seed rows from existing entries on first render
  ...
  const form = useForm<FormType>({
    ...
    mode: "onChange",
    defaultValues: {
      eosId: String(attendance.eosId),
      supervisorId: String(attendance.supervisorId),
      startDate: toInputDate(attendance.startDate),
      endDate: toInputDate(attendance.endDate),
    },
  });
  ...
  // Only regenerate rows when date range actually changes from the saved values
  useEffect(() => {
    if (!startDate || !endDate || startDate > endDate) return;

    const savedStart = toInputDate(attendance.startDate);
    const savedEnd = toInputDate(attendance.endDate);

    // If dates haven't changed from DB values, keep existing entry data
    if (startDate === savedStart && endDate === savedEnd) return;

    const dates = getWeekdays(startDate, endDate);
    setRows(
      dates.map((date) => {
        // Try to preserve existing check-in/out if same date exists
        const existing = attendance.entries.find(
          (e) => toInputDate(e.date) === toInputDate(date),
        );
        return {
          date,
  ...
  const onSubmit = (values: FormType) => {
    startTransition(async () => {
      try {
        await updateAttendanceListFull(attendance.id, values, rows);
        toast.success("Laporan absensi berhasil diperbarui");
          ...
          <div className="bg-muted/30 p-4 rounded-lg border space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-xs font-medium">Jam Masuk (Range)</label>
                  ...
                  {rows.map((row, i) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-1.5">
                        <CellInput
                          value={row.checkOut}
                          onChange={(v: string) => updateRow(i, "checkOut", v)}
                        />
                      </td>
                    </tr>
                  ))}
